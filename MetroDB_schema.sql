...
--
-- Host: localhost    Database: MetroDB
-- ------------------------------------------------------
...
) ENGINE=InnoDB AUTO_INCREMENT=14660 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=1097 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `js_status_mapping` (
  ...
  `line_t` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`js_code`),
...
CREATE TABLE `line_fleet` (
  ...
  KEY `line_fleet_ibfk_2` (`model_id`),
  CONSTRAINT `line_fleet_ibfk_1` FOREIGN KEY (`line_id`) REFERENCES `metro_lines` (`line_id`),
  CONSTRAINT `line_fleet_ibfk_2` FOREIGN KEY (`model_id`) REFERENCES `train_models` (`model_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=176316 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `metro_lines` (
  ...
  `express_operational` tinyint(1) DEFAULT 0 COMMENT 'Express route status: 1=functioning, 0=not functioning',
  PRIMARY KEY (`line_id`),
...
CREATE TABLE `metro_stations` (
  ...
  `express_state` varchar(20) DEFAULT NULL COMMENT 'Express service state',
  `route_color` enum('V','R','C') DEFAULT NULL COMMENT 'Route color: V=Green, R=Red, C=Blue',
  `platform_details` varchar(50) DEFAULT NULL COMMENT 'Platform details: island, side, mixed, etc.',
...
) ENGINE=InnoDB AUTO_INCREMENT=15304 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=111632 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Table structure for table `scheduled_status_overrides`
--

DROP TABLE IF EXISTS `scheduled_status_overrides`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
...
) ENGINE=InnoDB AUTO_INCREMENT=457714 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=4165 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=4132 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
/*!50001 VIEW `vw_lines_with_jsdata` AS select `ml`.`line_id` AS `line_id`,`ml`.`line_name` AS `line_name`,`ml`.`line_color` AS `line_color`,`ml`.`display_order` AS `display_order`,`ml`.`line_description` AS `line_description`,`ml`.`opening_date` AS `opening_date`,`ml`.`total_stations` AS `total_stations`,`ml`.`total_length_km` AS `total_length_km`,`ml`.`avg_daily_ridership` AS `avg_daily_ridership`,`ml`.`operating_hours_start` AS `operating_hours_start`,`ml`.`operating_hours_end` AS `operating_hours_end`,`ml`.`created_at` AS `created_at`,`ml`.`updated_at` AS `updated_at`,`ml`.`display_name` AS `display_name`,`ml`.`fleet_data` AS `fleet_data`,`ml`.`status_code` AS `status_code`,`ml`.`status_message` AS `status_message`,`ml`.`app_message` AS `app_message`,`ml`.`infrastructure` AS `infrastructure`,coalesce(`ml`.`status_code`,'1') AS `effective_status_code`,coalesce(`ml`.`status_message`,`ls`.`status_message`) AS `effective_status_message`,ifnull(`ml`.`infrastructure`,json_object('operationalStatus',if(`ls`.`status_id` is null,'operational','unknown'),'lastUpdated',current_timestamp(),'stationCodes',json_array())) AS `infrastructure_data` from (`metro_lines` `ml` left join `line_status` `ls` on(`ml`.`line_id` = `ls`.`line_id`)) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_station_status_history` AS select `ssh`.`history_id` AS `history_id`,`ssh`.`station_id` AS `station_id`,`ms`.`station_name` AS `station_name`,`ms`.`line_id` AS `line_id`,`ssh`.`status_type_id` AS `status_type_id`,`ost`.`status_name` AS `status_name`,`ssh`.`status_message` AS `status_message`,`ssh`.`last_updated` AS `last_updated`,`ssh`.`updated_by` AS `updated_by` from ((`station_status_history` `ssh` join `metro_stations` `ms` on(`ssh`.`station_id` = `ms`.`station_id`)) join `operational_status_types` `ost` on(`ssh`.`status_type_id` = `ost`.`status_type_id`)) order by `ssh`.`station_id`,`ssh`.`last_updated` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_stations_with_jsdata` AS select `ms`.`station_id` AS `station_id`,`ms`.`line_id` AS `line_id`,`ms`.`station_code` AS `station_code`,`ms`.`station_name` AS `station_name`,case when `ms`.`display_name` is not null then `ms`.`display_name` when `ms`.`station_name` regexp ' L[0-9A-Z]+$' then `ms`.`station_name` else concat(`ms`.`station_name`,' L',ucase(`ms`.`line_id`)) end AS `effective_display_name`,`ss`.`status_type_id` AS `status_type_id`,`ost`.`status_name` AS `status_name`,`ss`.`status_message` AS `status_message`,`ss`.`expected_resolution_time` AS `expected_resolution_time`,`ss`.`is_planned` AS `is_planned`,`ss`.`impact_level` AS `impact_level`,ifnull(`ms`.`access_details`,json_object()) AS `access_details`,`ms`.`transports` AS `transports`,`ms`.`services` AS `services`,`ms`.`accessibility` AS `accessibility`,`ms`.`commerce` AS `commerce`,`ms`.`amenities` AS `amenities`,`ms`.`image_url` AS `image_url`,`ml`.`line_color` AS `line_color` from (((`metro_stations` `ms` left join `metro_lines` `ml` on(`ms`.`line_id` = `ml`.`line_id`)) left join `station_status` `ss` on(`ms`.`station_id` = `ss`.`station_id`)) left join `operational_status_types` `ost` on(`ss`.`status_type_id` = `ost`.`status_type_id`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-08-25 16:29:37
